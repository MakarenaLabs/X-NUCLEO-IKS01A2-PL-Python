...
class LSM303AGR:
    # Sensori Accelerometro/magnetometro
    ...
    def LSM303AGR_ACC_ReadRegRaw(self, RegAddr, NumByteToRead):
        if (NumByteToRead > 1):
            RegAddr |= 0x80
        self.tx_buf[0] = RegAddr
        self._bus.send((((self.LSM303AGR_ACC_I2C_ADDRESS) >> 1) & 0x7F), self.tx_buf, 1, 0)
        raw = []
        for i in range(NumByteToRead):
            self._bus.receive((((self.LSM303AGR_ACC_I2C_ADDRESS) >> 1) & 0x7F), self.rx_buf, 1, 0)
            raw.append(self.rx_buf[0])
        return raw
    ...
    def LSM303AGR_ACC_WriteRegRaw(self, RegAddr, NumByteToRead, data):
        ...
        for i in data:
            self.tx_buf[0] = RegAddr
            self.tx_buf[1] = i
            self._bus.send((((self.LSM303AGR_ACC_I2C_ADDRESS) >> 1) & 0x7F), self.tx_buf, 2, 0)
            # time.sleep(0.02)
        raw = []
        for i in range(NumByteToRead):
            self._bus.receive((((self.LSM303AGR_ACC_I2C_ADDRESS) >> 1) & 0x7F), self.rx_buf, 1, 0)
    ...
    def LSM303AGR_MAG_ReadRegRaw(self, RegAddr, NumByteToRead):
        if (NumByteToRead > 1):
            RegAddr |= 0x80
        self.tx_buf[0] = RegAddr
        self._bus.send((((self.LSM303AGR_MAG_I2C_ADDRESS) >> 1) & 0x7F), self.tx_buf, 1, 0)
        raw = []
        for i in range(NumByteToRead):
            self._bus.receive((((self.LSM303AGR_MAG_I2C_ADDRESS) >> 1) & 0x7F), self.rx_buf, 1, 0)
            raw.append(self.rx_buf[0])
        return raw
    ...
    def LSM303AGR_MAG_WriteRegRaw(self, RegAddr, NumByteToRead, data):
        ...
        for i in data:
            self.tx_buf[0] = RegAddr
            self.tx_buf[1] = i
            self._bus.send((((self.LSM303AGR_MAG_I2C_ADDRESS) >> 1) & 0x7F), self.tx_buf, 2, 0)
            # time.sleep(0.02)
        raw = []
        for i in range(NumByteToRead):
            self._bus.receive((((self.LSM303AGR_MAG_I2C_ADDRESS) >> 1) & 0x7F), self.rx_buf, 1, 0)
